...
async function getOpenPort(port) {
  ...
  // const isReachable = await isPortReachable(port, { host: "109.123.242.59" });
...
async function getExamplesFromDirectory(port) {
  ...
  return exampleDirectories.map((example) => ({
    ...
    // url: `http://109.123.242.59:${port}/examples/${example}`,
  }));
}
...
// process.env.DEBUGGER_HUB_HTTP_URL = `http://109.123.242.59:${debuggerPort}/hub`;
...
// process.env.APP_URL = `http://109.123.242.59:${nextPort}`;

if (!process.env.FJS_MONOREPO) {
  const url = `http://localhost:${nextPort}`;
  // const url = `http://109.123.242.59:${nextPort}`;
